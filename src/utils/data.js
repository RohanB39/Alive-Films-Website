export const HeroData = [
  {
    src: "hero/person1.jpg",
    bg: "#0be4d1",
    delay: 0.1,
  },
  {
    src: "hero/person2.jpg",
    bg: "#fde490",
    delay: 0.3,
  },
  {
    src: "hero/person3.jpg",
    bg: "#00c9f7",
    delay: 0.2,
  },
  {
    src: "hero/person4.jpg",
    bg: "#83cfdf",
    delay: 0.2,
  },
  {
    src: "hero/person5.jpg",
    bg: "#fe8856",
    delay: 0.3,
  },
  {
    src: "hero/person6.jpg",
    bg: "#0be4d1",
    delay: 0.25,
  },
];

export const features = [
  {
    icon: "/features/Frame-0.png",
    title: "3x monthly revenue",
  },
  {
    icon: "/features/Frame-1.png",
    title: "Flat fee of 6-12%",
  },
  {
    icon: "/features/Frame-2.png",
    title: "Flexible repayments by revenue share",
  },
  {
    icon: "/features/Frame-3.png",
    title: "3-4 years repayment",
  },
  {
    icon: "/features/Frame-4.png",
    title: "$10-100k seed investment",
  },
  {
    icon: "/features/Frame-5.png",
    title: "2% of revenue up to 3X cap",
  },
];

export const ourDiffFeatures = [
  {
    icon: "/OurDiff/Frame-0.png",
    title: "Data Driven",
    des: "Our mission is to level the playing field for early stage growth capital. We provide capital that is unbiased, flexible and non dilutive with the execution support to accelerate value creation",
  },
  {
    icon: "/OurDiff/Frame-1.png",
    title: "flexible funding",
    des: "Funding that flexes as revenue grows with transparent costs and does not require dilution, personal guarantees or pitch decks",
  },
  {
    icon: "/OurDiff/Frame-2.png",
    title: "value accelerating",
    des: "On demand expertise to supercharge growth in areas that are hard to access for early stage founders",
  },
  {
    icon: "/OurDiff/Frame-3.png",
    title: "value accelerating",
    des: "On demand expertise to supercharge growth in areas that are hard to access for early stage founders",
  },
  {
    icon: "/OurDiff/Frame-4.png",
    title: "value accelerating",
    des: "On demand expertise to supercharge growth in areas that are hard to access for early stage founders",
  },
];

export const hitFeatures = [
  {
    icon: "/howItWorks/Frame-0.webp",
    title: "VISHAL NALE (Director & Producer)",
    des: "As the director crafts the vision, and the producer brings it to life, their collaboration forms the cinematic alchemy that captivates audiences and defines our creative journey.",
  },
  {
    icon: "/howItWorks/Frame-1.webp",
    title: "Rutuja Nagwekar (Creative Director)",
    des: "As the Creative Director, visionary innovation meets artistic flair, guiding the intersection of imagination and execution to shape compelling narratives and elevate our creative endeavors.",
  },
  {
    icon: "/howItWorks/Frame-2.webp",
    title: "Shraddha Gite (Art Director)",
    des: "As the Art Director, orchestrating a symphony of visuals, I bring creativity to life, weaving design elements into a seamless tapestry that captivates and communicates our aesthetic vision.",
  },
  {
    icon: "/howItWorks/Frame-3.webp",
    title: "Aakash Pisal (DOP)",
    des: "As the Director of Photography, I craft visual poetry, blending light, composition, and camera mastery to breathe life into the director's vision and elevate the storytelling experience on screen.",
  },
  {
    icon: "/howItWorks/Frame-4.webp",
    title: "Chintan P (Creative Director)",
    des: "As the Creative Director, I navigate the realms of imagination, steering teams to craft innovative and compelling narratives that resonate with our audience, forging the path to artistic brilliance.",
  },
];

export const testimonialsData = [
  {
    comment:
      "Working with ALIVE FILMS was a game-changer for our business. Their creativity and attention to detail elevated our promotional videos to a whole new level.",
    name: "John Smith",
    profession: "CEO of XYZ Corp",
  },
  {
    comment:
      "Our experience with ALIVE FILMS was nothing short of fantastic. They understood our vision and brought it to life with stunning visuals and engaging content.",
    name: "Cheryl R. Weiss",
    profession: "Marketing Manager at ABC Tech",
  },
  {
    comment:
      "The ALIVE FILMS team exceeded our expectations in capturing the essence of our conference. From pre-event planning to post-production, they demonstrated unmatched skill and dedication.",
    name: "Vicky R. Lane",
    profession: "Event Coordinator for DEF Conference",
  },
  {
    comment:
      "I can't thank [Video Production Company] enough for the outstanding promotional videos they produced for my real estate listings. Their cinematic approach showcased properties in a way that truly captured the attention of potential buyers.",
    name: "Jennifer Dubois",
    profession: "Nonprofit Director",
  },
];
